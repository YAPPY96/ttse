...
class TtsService : TextToSpeechService() {
    ...
    override fun onSynthesizeText(request: SynthesisRequest?, callback: SynthesisCallback?) {
        ...
        if (tts == null) {
            Log.e(TAG, "TTS engine not initialized, attempting to create...")
            TtsEngine.createTts(application)
        ...
        val ttsInstance = TtsEngine.tts!!

        Log.i(TAG, "text: $text, engineSpeed: $engineSpeed, lang: $language")
        ...
        callback.start(ttsInstance.sampleRate(), AudioFormat.ENCODING_PCM_16BIT, 1)
    ...
    private fun floatArrayToByteArray(audio: FloatArray): ByteArray {
        ...
        for (i in audio.indices) {
            // Clamp to [-1.0, 1.0] before converting to prevent distortion
            val clamped = audio[i].coerceIn(-1.0f, 1.0f)
            val sample = (clamped * 32767).toInt()
