...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Load native library on main thread BEFORE any background work or Compose rendering
        // to avoid race condition with hwui renderer (SIGABRT on destroyed mutex)
        try {
            System.loadLibrary("sherpa-onnx-jni")
            Log.i(TAG, "Native library loaded successfully on main thread")
        ...
        // Show UI immediately, load model in background
        setContent {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                Log.i(TAG, "Start to initialize TTS (async)")
                ...
                withContext(Dispatchers.Main) {
                    if (TtsEngine.tts != null) {
                        ...
                        loadError.value = "TTS model failed to initialize.\nCheck logcat for details."
                        isLoading.value = false
                    }
                }
            } catch (e: Exception) {
                    ...
                    CoroutineScope(Dispatchers.IO).launch {
                        ...
                        withContext(Dispatchers.Main) {
                            if (TtsEngine.tts != null) {
                                ...
                                loadError.value = "Retry failed. Check logcat."
                                isLoading.value = false
                            }
                        }
                    }
    ...
    @Composable
    private fun MainScreen(preferenceHelper: PreferencesHelper) {
        Scaffold(topBar = {
                            ...
                            onClick = {
                                ...
                                if (testText.isBlank() || testText.isEmpty()) {
                                    ...
                                    CoroutineScope(Dispatchers.Default).launch {
                                        try {
                                            val ttsInstance = TtsEngine.tts ?: return@launch
                                            ...
                                            val audioDuration =
                                                audio.samples.size / ttsInstance.sampleRate()
                                                    .toFloat()
                                            val RTF = String.format(
                                                "Number of threads: %d\nElapsed: %.3f s\nAudio duration: %.3f s\nRTF: %.3f/%.3f = %.3f",
                                                ttsInstance.config.model.numThreads,
                                                elapsed,
                                                audioDuration,
                                                elapsed,
                                                audioDuration,
                                            ...
                                            if (ok) {
                                                ...
                                                withContext(Dispatchers.Main) {
                                                    startEnabled = true
                                                    Toast.makeText(applicationContext, "Audio generation returned empty result", Toast.LENGTH_SHORT).show()
                                                }
                                            }
                                        } catch (e: Exception) {
    ...
    private fun initAudioTrack() {
        val sampleRate = TtsEngine.tts?.sampleRate() ?: 44100
